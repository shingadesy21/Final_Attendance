...
public class BE_CSE_C_MIS_ItemAdapter extends RecyclerView.Adapter<BE_CSE_C_MIS_ItemAdapter.ViewHolder> {
    private Context context;
    private List<BE_CSE_C_MIS_Dataitem> items;
    FirebaseDatabase database;
    DatabaseReference reference;

    public BE_CSE_C_MIS_ItemAdapter(Context context, List<BE_CSE_C_MIS_Dataitem> items) {
    ...
    @NonNull
    @Override
    public BE_CSE_C_MIS_ItemAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view= LayoutInflater.from(context).inflate(R.layout.be_cse_c_stud_info,parent,false);
        return new BE_CSE_C_MIS_ItemAdapter.ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull final BE_CSE_C_MIS_ItemAdapter.ViewHolder holder, int position) {
        final BE_CSE_C_MIS_Dataitem dataitem=items.get(position);
        ...
        holder.aSwitch.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                if(isChecked)
                {
                    Query query= FirebaseDatabase.getInstance().getReference("Present Stud Data").child("CSE").child("BE").child("C").child(currentdate).child("MIS").orderByChild("roll_no").equalTo(stud_RollNo);
                    ...
                    query.addListenerForSingleValueEvent(new ValueEventListener() {
                        ...
                        public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                            ...
                            if(dataSnapshot.getChildrenCount()>0) {
                                ...
                                reference =  database.getReference("Presenty Data").child("CSE").child("BE").child("C").child(currentdate).child("MIS");
                    ...
                    query.addListenerForSingleValueEvent(new ValueEventListener() {
                        @Override
                        public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                            reference = (DatabaseReference) database.getReference("Presenty Data").child("CSE").child("BE").child("C").child(currentdate).child("MIS");
                            database = FirebaseDatabase.getInstance();
